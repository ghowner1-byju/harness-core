...
public class AdoCreateFileScmApiErrorHandlerTest extends GitSyncTestBase {
  @Inject AdoCreateFileScmApiErrorHandler adoCreateFileScmApiErrorHandler;
  ...
  public void testHandleErrorOnUnauthorizedResponse() {
    try {
      adoCreateFileScmApiErrorHandler.handleError(203, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
  ...
  public void testHandleErrorOnResourceNotFoundResponse() {
    try {
      adoCreateFileScmApiErrorHandler.handleError(404, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
  ...
  public void testHandleErrorOnBadRequestEntityResponse() {
    try {
      adoCreateFileScmApiErrorHandler.handleError(400, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
      WingsException exception = ExceptionUtils.cause(ScmBadRequestException.class, ex);
      assertThat(exception).isNotNull();
      assertThat(exception.getMessage()).isEqualTo(errorMessage);
  ...
  public void testHandleErrorWhenUnexpectedStatusCode() {
    try {
      adoCreateFileScmApiErrorHandler.handleError(405, errorMessage, ErrorMetadata.builder().build());
    } catch (Exception ex) {
